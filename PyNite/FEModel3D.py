...
#%%
    def AddMember(self, Name, iNode, jNode, E, G, Iy, Iz, J, A, auxNode=None):
        ...
        Iy : number
            The polar moment of inertia of the member about its local y-axis.
        Iz : number
            The polar moment of inertia of the member about its local z-axis.
        J : number
...
#%%
    def AddPlate(self, Name, iNode, jNode, mNode, nNode, t, E, μ):
        '''
        ...
        # Create a new member
        newPlate = Plate3D(Name, self.GetNode(iNode), self.GetNode(jNode), self.GetNode(mNode), self.GetNode(nNode), t, E, μ)
...
#%%            
    def AddNodeDisplacement (self, Node, Direction, Magnitude): 
        ...
        Direction : {'DX', 'DY', 'DZ', 'RX', 'RY', 'RZ'}
            The global direction the nodal displacement is being applied. Displacements are 'DX', 'DY', and 'DZ'. Rotations are 'RX', 'RY', and 'RZ'.
...
#%%
    def AddNodeLoad(self, Node, Direction, P, case='Case 1'):
        ...
        Direction : {'FX', 'FY', 'FZ', 'MX', 'MY', 'MZ'}
            The global direction the load is being applied. Forces are 'FX', 'FY', and 'FZ'. Moments are 'MX', 'MY', and 'MZ'.
        P : number
...
#%%      
    def AddMemberPtLoad(self, Member, Direction, P, x, case='Case 1'):
        ...
        ----------
        Member : string
            The name of the member where the load is being applied.
        Direction : {'Fx', 'Fy', 'Fz', 'Mx', 'My', 'Mz'}
            The direction in which the force is to be applied. Note that
            typical beam sign convention is used.
            Transverse forces acting toward the beam are positive.
            Moments are positive if they act counter-clockwise relative to the beam's local coordinate system.
            Torsional point loads follow the right hand rule for sign convention.
        P : number
            The magnitude of the load.
        x : number
            The load's location along the member's local x-axis, starting at the i-node.
        case : string
            The name of the load case the load belongs to.
        '''
...
#%%
    def AddMemberDistLoad(self, Member, Direction, w1, w2, x1=None, x2=None, case='Case 1'):
        ...
        ----------
        Member : string
            The name of the member where the load is being appied.
        Direction : {'Fx', 'Fy', 'Fz'}
...
#%%
    def __Renumber(self):
        '''
        Assigns node, member, and plate IDs to be used internally by the
        program. Numbers are assigned according to the order the nodes, members, and plates
...
#%%
    def __AuxList(self):
        '''
        Builds a list with known nodal displacements with the positions of
        known and unknown nodal displacements in global stiffness matrix.
...
#%%    
    def K(self):
        ...
        for member in self.Members:
            ...
            # 'm' & 'n' below are corresponding row/column indices in the global stiffness matrix
            for a in range(12):
